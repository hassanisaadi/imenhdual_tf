...
def imdualenhancer_train(imdualenhancer, lr):
    with load_data(fxl='./data/mb2014_bin/pa_L_p8_b16_da1.npy',
                   fxr='./data/mb2014_bin/pa_R_p8_b16_da1.npy',
                   fyl='./data/mb2014_bin/gt_L_p8_b16_da1.npy') as data:
        ...
        eval_dataXL = load_images(eval_filesXL)  # list of array of different size, 4-D, pixel value range is 0-255
        eval_dataXR = load_images(eval_filesXR)
        eval_dataYL = load_images(eval_filesYL)
        imdualenhancer.train(XL, XR, YL, eval_dataXL,
                eval_dataXR, eval_dataYL,
...
def main(_):
    ...
    if args.use_gpu:
        ...
        with tf.Session(config=tf.ConfigProto(gpu_options=gpu_options)) as sess:
            model = imdualenhancer(sess, args.n, args.v, K, args.batch_size)
            if args.phase == 'train':
                imdualenhancer_train(model, lr=lr)
